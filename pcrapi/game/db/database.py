...
import typing
from pcrapi.game.model.enums import eCampaignCategory
from pcrapi.game.model.common import UnitData, eInventoryType, RoomUserItem, InventoryInfo
from pcrapi.game.model.custom import ItemType
import datetime
from collections import Counter, defaultdict
from .dbmgr import dbmgr
from .methods import *
from .models import *
from pcrapi.util.linq import flow
from queue import SimpleQueue
from .constdata import extra_drops
from pcrapi.core.apiclient import apiclient
...
class database():
        ...
        with dbmgr.session() as db:
            ...
            self.normal_quest_rewards: Dict[int, typing.Counter[ItemType]] = (
                ...
                .to_dict(lambda x: x.quest_id, lambda x:
                    ...
                    .where(lambda y: y != 0)
                         .select_many(
                        lambda y: self.wave_groups[y].get_drop_reward_ids() if y in self.wave_groups else [])
                    .where(lambda y: y != 0)
            ...
            self.ex_equipment_type_to_clan_battle_ex: Dict[int, int] = {  # 只有每个类别的会战装备
                ex.category: ex.ex_equipment_id for ex in self.ex_equipment_data.values() if
                ex.clan_battle_equip_flag == 1 and ex.rarity == 3
            }
    ...
    def get_ex_equip_star_from_pt(self, id: int, pt: int) -> int:
        ...
        history_star = [star for star, enhancement_data in self.ex_equipment_enhance_data[rarity].items() if
                        enhancement_data.total_point <= pt]
    ...
    def get_inventory_name(self, item: InventoryInfo) -> str:
        try:
            if item.type == eInventoryType.ExtraEquip:
                return f"{self.get_ex_equip_rarity_name(item.id)}{item.ex_equip.rank}-" + self.inventory_name[
                    (item.type, item.id)]
    ...
    def get_ex_equip_name(self, item: int, rank: int = 0) -> str:
        try:
            return f"{self.ex_rarity_name[self.ex_equipment_data[item].rarity]}{rank}-" + self.inventory_name[
                (eInventoryType.ExtraEquip, item)]
        except:
    ...
    def is_equip(self, item: ItemType, uncraftable_only: bool = False) -> bool:
        return item[0] == eInventoryType.Equip and item[1] >= 101000 and item[1] < 140000 and (
                    not uncraftable_only or not self.is_equip_craftable(item))
    ...
    def is_campaign(self, campaign: str, now: Union[None, datetime.datetime] = None, level: int = 999) -> bool:
        ...
        n3 = (flow(self.campaign_schedule.values())
              .where(lambda x: self.is_normal_quest_campaign(
            x.id) and x.value >= 3000 and self.is_level_effective_scope_in_campaign(level, x.id))
    ...
    def parse_time(self, time: str) -> datetime.datetime:
        for timeformat in ['%Y/%m/%d %H:%M:%S', '%Y/%m/%d %H:%M', '%Y/%m/%d', '%Y-%m-%dT%H:%M:%S.%fZ',
    ...
    def get_unique_equip_material_demand(self, unit_id: int, slot_id: int, start_rank: int, target_rank: int) -> \
    typing.Counter[ItemType]:
    ...
    def get_rank_promote_equip_demand(self, unit_id: int, start_rank: int, start_rank_equip_slot: List[bool], target_rank, target_rank_equip_slot: List[bool]) -> typing.Counter[ItemType]: # 都是整装
        ret = (
            ...
            .select(lambda x: x[1])
             .sum(seed=Counter())) +
            Counter((eInventoryType(eInventoryType.Equip),
                     int(getattr(self.unit_promotion[unit_id][target_rank], f"equip_slot_{i}"))) for i in range(1, 7) if
                    target_rank_equip_slot[i - 1]) -
            Counter((eInventoryType(eInventoryType.Equip),
                     int(getattr(self.unit_promotion[unit_id][start_rank], f"equip_slot_{i}"))) for i in range(1, 7) if
                    start_rank_equip_slot[i - 1])
            )
        return ret
    ...
    def get_equip_raw_ore(self, equip_id: int) -> ItemType:
        ...
        return self.equip_promotion_to_raw_ore[promote_level] if promote_level in self.equip_promotion_to_raw_ore else (
        eInventoryType.Equip, equip_id)
    ...
    def get_unique_equip_rank_from_level(self, equip_slot: int, level: int):
        rank = self.unique_equipment_enhance_data[equip_slot][level].rank if level in \
                                                                             self.unique_equipment_enhance_data[
                                                                                 equip_slot] else 1
        return rank
    ...
    def get_unique_equip_pt_from_level(self, equip_slot: int, level: int):
        pt = self.unique_equipment_enhance_data[equip_slot][level].total_point if level in \
                                                                                  self.unique_equipment_enhance_data[
                                                                                      equip_slot] else 0
        return pt
    ...
    def get_travel_quest_id_from_candidate(self, candidate: str) -> int:
        ...
        ret = next(x.travel_quest_id for x in self.travel_quest_data.values() if
                   x.travel_area_id % 10 == int(area) and x.travel_quest_id % 10 == int(quest))
        return ret
    ...
    def calc_unit_attribute(self, unit_data: UnitData, read_story: Set[int]) -> UnitAttribute:
        ...
        # 等级属性
        base_attribute += self.unit_rarity[unit_id][rarity].get_unit_attribute_growth(
            unit_data.unit_level + promotion_level)
        ...
        for equip in unit_data.equip_slot:
            if equip.is_slot:
                equip_attribute += (self.equip_data[equip.id].get_unit_attribute() + self.equipment_enhance_rate[
                    equip.id].get_unit_attribute(equip.enhancement_level)).ceil()
    ...
    def calc_unit_attribute_power(self, unit_data: UnitData, read_story: Set[int],
                                  coefficient: UnitStatusCoefficient) -> float:
