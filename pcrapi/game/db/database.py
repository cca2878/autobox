...
from pcrapi.game.model.common import RoomUserItem, InventoryInfo
import datetime
...
class database:
    ...
    def update(self, dbmgr: dbmgr):
        ...
        with dbmgr.session() as db:
            ...
            self.guild_story: List[StoryDetail] = (
                ...
                .where(lambda x: 3000000 <= x.story_id < 4000000)
                .to_list()
            )

            self.main_story: List[StoryDetail] = (
                StoryDetail.query(db)
                .where(lambda x: 2000000 <= x.story_id < 3000000)
                .to_list()
            )
            ...
            self.unit_story: List[StoryDetail] = (
                StoryDetail.query(db)
                .where(lambda x: 1000000 <= x.story_id < 2000000)
            ...
            self.love_cake: List[ItemDatum] = (
                ItemDatum.query(db)
                .where(lambda x: 50000 <= x.item_id < 51000)
            ...
            self.exp_potion: List[ItemDatum] = (
                ItemDatum.query(db)
                .where(lambda x: 20001 <= x.item_id < 21000)
                .to_list()
            )
            ...
            self.equip_enhance_stone: List[ItemDatum] = (
                ItemDatum.query(db)
                .where(lambda x: 22001 <= x.item_id < 23000)
                .to_list()
            )
    ...
    def is_exp_upper(self, item: ItemType) -> bool:
        return item[0] == eInventoryType.Item and 20000 <= item[1] < 21000

    def is_equip_upper(self, item: ItemType) -> bool:
        return item[0] == eInventoryType.Item and 22000 <= item[1] < 23000

    def is_unit_memory(self, item: ItemType) -> bool:
        return item[0] == eInventoryType.Item and 31000 <= item[1] < 32000

    def is_unit_pure_memory(self, item: ItemType) -> bool:
        return item[0] == eInventoryType.Item and 32000 <= item[1] < 33000

    def is_equip(self, item: ItemType, uncraftable_only: bool = False) -> bool:
        return item[0] == eInventoryType.Equip and 101000 <= item[1] < 140000 and (not uncraftable_only or not self.is_equip_craftable(item))

    def is_equip_raw_ore(self, item: ItemType) -> bool:
        return item[0] == eInventoryType.Equip and 150001 <= item[1] < 160000
    ...
    def get_active_hatsune(self) -> List[HatsuneSchedule]:
        ...
        return flow(self.hatsune_schedule.values()) \
                .where(lambda x: self.parse_time(x.start_time) <= now <= self.parse_time(x.end_time)) \
                .to_list()
    ...
    def get_open_hatsune(self) -> List[HatsuneSchedule]:
        now = datetime.datetime.now()
        return flow(self.hatsune_schedule.values()) \
                .where(lambda x: self.parse_time(x.start_time) <= now <= self.parse_time(x.close_time)) \
                .to_list()

    def get_active_seasonpass(self) -> List[SeasonpassFoundation]:
        now = datetime.datetime.now()
        return flow(self.seasonpass_foundation.values()) \
                .where(lambda x: self.parse_time(x.start_time) <= now <= self.parse_time(x.limit_time)) \
                .to_list()
    ...
    def get_rank_promote_equip_demand(self, unit_id: int, start_rank: int, start_rank_equip_slot: List[bool], target_rank, target_rank_equip_slot: List[bool]) -> typing.Counter[ItemType]: # 都是整装
        ret = (
            ...
            .where(lambda x: start_rank <= x[0] < target_rank)
            .select(lambda x: x[1])
            .sum(seed=Counter())) - 
