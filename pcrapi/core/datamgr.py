...
import typing
from pcrapi.game.model.common import *
from pcrapi.game.model.custom import ItemType
import json, base64, gzip
from pcrapi.game.db.assetmgr import instance as assetmgr
from pcrapi.game.db.dbmgr import instance as dbmgr
from pcrapi.game.db.database import db
from pcrapi.game.db.models import ItemDatum, TrainingQuestDatum
...
class datamgr(BaseModel, Component[apiclient]):
    ...
    def update_stamina_recover(self):
        ...
        now = apiclient.time
        remain = max(0,
                     self.stamina_full_recovery_time - now + self.settings.quest.recovery_time - 1) // self.settings.quest.recovery_time
    ...
    def get_campaign_times(self, condition_func) -> int:
        times = [db.get_campaign_times(campaign_id) for campaign_id in self.campaign_list if
                 condition_func(campaign_id) and db.is_level_effective_scope_in_campaign(self.team_level, campaign_id)]
        if not times:
            return 0
        times = max(times)
        return int(times)
    ...
    def get_unique_equip_material_demand(self, equip_slot: int, unit_id: int, token: ItemType,
                                         target_rank: int = -1) -> int:
        start_rank = self.unit[unit_id].unique_equip_slot[0].rank if unit_id in self.unit and self.unit[
            unit_id].unique_equip_slot else 0
        demand = db.get_unique_equip_material_demand(unit_id, equip_slot, start_rank, db.unique_equipment_max_rank[
            equip_slot] if target_rank == -1 else target_rank)
    ...
    def get_unit_eqiup_demand(self, unit_id: int) -> typing.Counter[ItemType]:
        ...
        equips = db.get_rank_promote_equip_demand(unit_id, rank, equip_slot, db.equip_max_rank,
                                                  db.equip_max_rank_equip_slot)
    ...
    def get_rarity_memory_demand(self, unit_id: int, token: ItemType, rarity_slot_num: int,
                                 target_rarity: int = 999) -> int:
        rarity = -1
    ...
    def get_library_unit_data(self) -> List:
        ...
        for unit in self.unit.values():
            ...
            result.append({ 
                ...
                "q": str(
                    db.unique_equip_rank[1][unit.unique_equip_slot[0].rank].enhance_level) if unit.unique_equip_slot and
    ...
    def get_unit_memory_demand(self, unit_id: int, target_rarity: int = 999, target_unique_rank: int = -1,
                               exceed_level: bool = True) -> int:
        memory_id = db.unit_to_memory[unit_id]
        token = (eInventoryType.Item, memory_id)
        need = self.get_rarity_memory_demand(unit_id, token, 2, target_rarity) + self.get_unique_equip_memory_demand(
            unit_id, token, target_unique_rank) + self.get_exceed_level_unit_demand(unit_id, token) * exceed_level
        return need
    ...
    def get_clan_ex_equip_demand(self, start_rank: Union[None, int] = None, like_unit_only: bool = False) -> \
    ...
    def get_clan_ex_equip_gap(self, start_rank: Union[None, int] = None, like_unit_only: bool = False) -> \
    typing.Counter[ItemType]:
    ...
    def get_not_enough_item(self, demand: typing.Counter[ItemType]) -> typing.Counter[ItemType]:
        bad = Counter(
            {item: cnt - self.get_inventory(item) for item, cnt in demand.items() if cnt > self.get_inventory(item)})
        return bad
